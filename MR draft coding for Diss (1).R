...
#read in data 
diss_raw <- read.csv("katalina_dat.csv")
head(diss_raw)

d_group <- read.csv("data_new_wed.csv")

#add the new group varible 
diss_raw <- d_group

merged_data <- cbind(dataset1, dataset2)

diss_data <- cbind(diss_raw, d_group)
...
catdig <- c(diss_data$group)
catdig_count <-table(catdig)
print(catdig_count)

catdig2 <- c(d_group$group)
catdig_count2 <-table(catdig)
print(catdig_count2)
...
(cor(diss_raw$roar_acc, diss_raw$arhq))
